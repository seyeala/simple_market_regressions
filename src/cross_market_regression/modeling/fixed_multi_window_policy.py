...
class FixedMultiWindowUtilityPolicy(tf.keras.Model):
    ...
    def psi(self, inputs: Any) -> tf.Tensor:
        ...
        ohlcv, current_position, time_to_close = self._unpack_inputs(inputs)
        g = self.state_vector(ohlcv, current_position, time_to_close)
    ...
    def state_vector(self, ohlcv: Any, current_position: Any, time_to_close: Any) -> tf.Tensor:
        ...
        mu_4 = self._last_ewma(returns, 4)
        mu_16 = self._last_ewma(returns, 16)
        mu_60 = self._last_ewma(returns, 60)
        i_8 = self._last_ewma(intrabar, 8)
        i_32 = self._last_ewma(intrabar, 32)
        sigma_4 = self._last_ew_std(returns, 4)
        sigma_60 = self._last_ew_std(returns, 60)
        r_16 = close[:, -1] / (self._last_ewma(close, 16) + eps) - 1.0
        r_60 = close[:, -1] / (self._last_ewma(close, 60) + eps) - 1.0
        e_16 = self._last_ewma(energy, 16)
        e_60 = self._last_ewma(energy, 60)
    ...
    def legal_action_mask(self, inputs: Any, batch_size: tf.Tensor | None = None) -> tf.Tensor:
        ...
        _, current_position, _ = self._unpack_inputs(inputs)
    ...
    def _last_ewma(self, values: tf.Tensor, window: int) -> tf.Tensor:
        weighted = self._causal_exponential_window(values, window)
        return weighted[:, -1]

    def _last_ew_std(self, values: tf.Tensor, window: int) -> tf.Tensor:
        mean = self._causal_exponential_window(values, window)
        var = self._causal_exponential_window(tf.square(values - mean), window)
        return tf.sqrt(tf.maximum(var[:, -1], 0.0))

    def _causal_exponential_window(self, values: tf.Tensor, window: int) -> tf.Tensor:
    ...
    def _unpack_inputs(self, inputs: Any) -> tuple[tf.Tensor, tf.Tensor, tf.Tensor]:
        if isinstance(inputs, Mapping):
            ...
            return tf.convert_to_tensor(ohlcv, tf.float32), current_position, time_to_close
        ohlcv = tf.convert_to_tensor(inputs, tf.float32)
        batch = tf.shape(ohlcv)[0]
        return ohlcv, tf.zeros([batch], tf.float32), tf.ones([batch], tf.float32)
